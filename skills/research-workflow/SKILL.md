...
Running a research pipeline on "transformer efficiency":

```
User: "Design a research plan for improving transformer inference efficiency."

1. Hypothesis Formation
   "We hypothesize that structured pruning of attention heads at 40% sparsity
   will maintain >=98% of baseline BLEU on WMT-14 EN-DE while reducing
   inference latency by >=30% on A100 GPU."

2. Baseline Definition
   - Simple baseline: Unmodified transformer-base (no pruning)
   - Competitive baseline: Current SOTA — CoFi pruning (Xia et al., 2022)
   - Ablated: Head pruning only, FFN pruning only, combined

3. Evaluation Protocol
   - Primary metric: BLEU on WMT-14 EN-DE test set
   - Secondary metrics: Inference latency (ms/sentence), FLOPs, peak memory
   - Statistical plan: 5 seeds, paired t-test, p < 0.05

4. Experiment Tracking
   Run ID: pruned-wmt14-headffn-20260316-a1b2c3
   Config: sparsity=0.4, method=structured, schedule=cubic, epochs=10
   Environment: Python 3.11, PyTorch 2.2, CUDA 12.1, 1xA100-80GB

5. Roadmap: Foundation (week 1-2) → Core pruning experiments (week 3-5)
   → Ablations + significance tests (week 6-7) → Write-up (week 8)
```
